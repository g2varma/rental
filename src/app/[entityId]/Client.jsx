...
const Client = async ({ entityId, searchParams }) => {
  const {
    page,
    size,
    type,
  ...
  const listingsRes = await getListings(
    ...
    size ?? 10,
    type,
    null, // propertyType not required
    priceL,
    priceH,
    bedrooms,
    bathrooms,
    ...
    sortKey,
    sortValue,
    null, // searchKey not required
    entityId // Using entityId as `client_id`
  );
  ...
  const sanitizedListings = listingsRes?.properties
    ? listingsRes.properties.map((property) => ({
      ...
      client_image_url: property.client_image_url || property.client_name,
  ...
  return (
    ...
    <div>
      {/* Main Header Nav */}
      <DefaultHeader data={convertedListings?.[0]?.ClientImageUrl
        ? convertedListings[0].ClientImageUrl
        : null}
      />
      {/* End Main Header Nav */}
      {/* Mobile Nav  */}
      <MobileMenu data={convertedListings?.[0]?.ClientImageUrl
        ? convertedListings[0].ClientImageUrl
        : null} />
      {/* End Mobile Nav  */}
      <Wrapper
        heading={`Client View: ${entityId}`}
