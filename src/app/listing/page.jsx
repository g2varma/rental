...
const ListingPage = async ({ searchParams, homeConfig }) => {
  const {
    page,
    size,
    type,
  ...
  // Decode the 'type' query parameter and clean it up
  let headingType = "All Properties";
  if (type) {
    const decodedType = decodeURIComponent(type); // Decode the encoded type
    const typeArray = decodedType
      .split(",") // Split by commas
      .filter((item) => item.toLowerCase() !== "null" && isNaN(item)) // Remove "null" and numeric values
      .map(
        (word) =>
          word.charAt(0).toUpperCase() + word.slice(1).replace(/-/g, " ") // Capitalize and replace hyphens with spaces
      );

    headingType = typeArray.length > 0 ? typeArray.join(", ") : "All Properties";
  }
